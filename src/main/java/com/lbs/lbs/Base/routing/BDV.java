...
//				System.out.println("\ncommonNodeID :" + commonNodeID);
	...
	// test the third condition: uniformly bounded stretch
	public boolean UBS(AlternativePaths<V, E> path) {
		//if the path is already tested: just return the result
		if (path.UBStested) {
			return path.passUBS;
		} // if not: initialize the variable with true
		else {
			path.UBStested = true;
		}
		
		if (path.NODE_X == null || path.NODE_Y == null || path.length_x == 0  || path.length_y == 0) {
			path.passUBS = false;
			return path.passUBS;
		}
		// run the Dijkstra from x- to y-node to get the shortest path between x and y
		double shortestLengthFromXtoY = this.dj.run(path.NODE_X,path.NODE_Y);
		
		//the path between x and y should not be longer than the shortest path between x and y + this.epsilon * (the shortest path between x and y)
	...
	public boolean localOptimality(AlternativePaths<V, E> path) throws Exception {
		...
		//?
		if (NODE_X == null || NODE_Y == null || viaNode_F == null || viaNode_B == null ) {		
			path.passLocalOptimality = false;
			return path.passLocalOptimality;
		}
		// run the dijkstra to get the shortest path between x or y and the via node
