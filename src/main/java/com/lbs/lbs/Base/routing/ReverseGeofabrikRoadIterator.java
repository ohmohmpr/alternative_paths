 ...
 */
public class ReverseGeofabrikRoadIterator<V, E extends WeightedArcData> implements NodeIterator<V, E> {

	protected DiGraphArc<V, E> currArc;
	protected NodeIterator<V, E> addIt;
	protected boolean start;

	public ReverseGeofabrikRoadIterator(NodeIterator<V, E> additionalIterator) {
	...
	public Iterator<DiGraphNode<V, E>> getIterator(DiGraphNode<V, E> s) {
		LinkedList<DiGraphArc<V, E>> incomingArcs = new LinkedList<>();

		String fclass;
		for (var arc : s.getIncomingArcs()) {
		...
		Iterator<DiGraphArc<V, E>> it = incomingArcs.iterator();
